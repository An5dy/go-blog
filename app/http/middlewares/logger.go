package middlewares

import (
	"bytes"
	"go-blog/pkg/helpers"
	"go-blog/pkg/logger"
	"io/ioutil"
	"time"

	"github.com/gin-gonic/gin"
	"github.com/spf13/cast"
	"go.uber.org/zap"
)

// responseBodyWriter 响应结构体，用于获取请求响应数据
type responseBodyWriter struct {
	gin.ResponseWriter
	body  *bytes.Buffer // 响应数据
	start time.Time     // 请求开始时间
}

// Write 写入响应
func (r responseBodyWriter) Write(b []byte) (int, error) {
	r.body.Write(b)
	return r.ResponseWriter.Write(b)
}

// Logger 记录请求日志
func Logger() gin.HandlerFunc {
	return func(c *gin.Context) {
		// 设置 response writer
		w := &responseBodyWriter{
			body:           &bytes.Buffer{},
			ResponseWriter: c.Writer,
		}
		c.Writer = w

		// 获取请求数据
		requestBody := getRequestBody(c)

		// 设置开始时间
		w.start = time.Now()

		// 处理请求，返回响应结果
		c.Next()

		// 记录日志
		setLogger(requestBody, w, c)
	}
}

// getRequestBody 获取请求数据
func getRequestBody(c *gin.Context) (requestBody []byte) {
	// c.Request.Body 是一个 buffer 对象，只能读取一次
	requestBody, _ = ioutil.ReadAll(c.Request.Body)
	// 读取后，重新赋值 c.Request.Body ，以供后续的其他操作
	c.Request.Body = ioutil.NopCloser(bytes.NewBuffer(requestBody))
	return
}

// setLogger 设置日志记录
func setLogger(r []byte, w *responseBodyWriter, c *gin.Context) {
	responseStatus := w.Status()

	// 设置访问日志格式
	logFields := []zap.Field{
		zap.Int("status", responseStatus),
		zap.String("time", w.start.Format("2006/01/02 - 15:04:05")),
		zap.String("request", c.Request.Method+" "+c.Request.URL.String()),
		zap.String("query", c.Request.URL.RawQuery),
		zap.String("ip", c.ClientIP()),
		zap.String("user-agent", c.Request.UserAgent()),
		zap.String("error", c.Errors.ByType(gin.ErrorTypePrivate).String()),
		zap.String("latency", helpers.MicrosecondsStr(time.Since(w.start))),
	}
	if c.Request.Method == "POST" || c.Request.Method == "PUT" || c.Request.Method == "DELETE" {
		// 请求的内容
		logFields = append(logFields, zap.String("Request Body", string(r)))
		// 响应的内容
		logFields = append(logFields, zap.String("Response Body", w.body.String()))
	}

	// 设置响应类型
	if responseStatus > 400 && responseStatus <= 499 {
		// 除了 StatusBadRequest 以外，warning 提示一下，常见的有 403 404，开发时都要注意
		logger.Warn("HTTP Warning "+cast.ToString(responseStatus), logFields...)
	} else if responseStatus >= 500 && responseStatus <= 599 {
		// 除了内部错误，记录 error
		logger.Error("HTTP Error "+cast.ToString(responseStatus), logFields...)
	} else {
		logger.Debug("HTTP Access Log", logFields...)
	}
}
